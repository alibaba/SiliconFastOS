...
func outputKernelInitrd(base string, kernel []byte, initrd []byte, cmdline string, ucode []byte) error {
	...
	if len(ucode) != 0 {
		log.Infof("  %s ucode+%s %s", base+"-kernel", base+"-initrd.cpio.gz", base+"-cmdline")
		if err := os.WriteFile(base+"-initrd.cpio.gz", ucode, os.FileMode(0644)); err != nil {
			return err
		}
		if len(initrd) != 0 {
			f, err := os.OpenFile(base+"-initrd.cpio.gz", os.O_APPEND|os.O_WRONLY, 0644)
			if err != nil {
				return err
			}
			defer f.Close()
		...
		if len(initrd) != 0 {
			log.Infof("  %s %s %s", base+"-kernel", base+"-initrd.cpio.gz", base+"-cmdline")
			if err := os.WriteFile(base+"-initrd.cpio.gz", initrd, os.FileMode(0644)); err != nil {
				return err
			}
		}
	}
